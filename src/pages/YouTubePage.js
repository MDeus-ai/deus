...
import { FaExpand, FaExternalLinkAlt, FaEye, FaThumbsUp, FaCalendarAlt, FaComment, FaTimes } from 'react-icons/fa';
import { format } from 'date-fns';

// Custom Tabs Components with mobile optimization
...
const Tab = ({ children, isActive, onClick }) => (
  <button
    onClick={onClick}
    className={`px-4 py-2 text-base md:text-lg whitespace-nowrap transition-colors duration-200 ${
      isActive 
  ...
  const EngagementCard = ({ title, value, description, showProgress }) => (
   ...
   return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 text-gray-100">
      ...
      {/* Video Modal */}
      {expandedVideo && <VideoModal video={expandedVideo} onClose={() => setExpandedVideo(null)} />}
    </div>
  );
};
