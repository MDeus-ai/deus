...
const navItems = [
    ...
    { label: "Blog Posts", href: "/blog", dropdown:
      [ { title: "The Ultimate Guide to Quantization with PyTorch and ONNX",
          description: "Learn why, what, and how to shrink deep learning models. A complete tutorial with code examples, advanced concepts, and a real-world case study on the PlantVision project.", href: "/blog/quantization", tag: "New"},
       { title: "Continuous Integration (CI) and Unit Testing: Lessons from Building PlantVision",
          description: "How automation and testing helped me catch bugs early and move fast", href: "/blog/ci-unit-tests"},
        { title: "The Anatomy of a Professional Machine Learning Project: A Guide to Structure and Sanity",
          description: "A comprehensive guide to understanding how real-world machine learning projects are structured", href: "/blog/ml-project-structure"},
        { title: "Just-In-Time Learning Framework",
          description: "My new framework for learning concepts and working on projects", href: "/blog/just_in_time_learning"},
        { title: "Batch Normalization Crush Course",
          description: "Understanding how batch normalization helps NNs", href: "/blog/BatchNormalization" },
      ]
    },
...
const DropdownMenu = ({ items, onLinkClick }) => (
  ...
  >
    <div className="bg-surface text-text-primary rounded-md shadow-2xl border border-border/20 min-w-[520px] max-w-[580px] sm:min-w-[480px] sm:max-w-[520px]">
      {/* Header with padding */}
      <div className="p-3">
        {/* Scrollable content area with max height */}
        <div className="max-h-[75vh] sm:max-h-[65vh] overflow-y-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-border/30 hover:scrollbar-thumb-border/50">
          {items.map((item, index) => (
            <React.Fragment key={index}>
              <Link
                to={item.href}
                className="block p-4 hover:bg-white/10 dark:hover:bg-white/5 rounded-md transition-colors"
                onClick={onLinkClick}
              >
                <div className="flex items-center justify-between gap-4">
                  <p className="font-bold text-text-primary text-base sm:text-lg leading-tight">{item.title}</p>
                  {item.tag && (
                    <span className="whitespace-nowrap bg-accent text-accent-text text-sm font-bold px-3 py-1.5 rounded flex-shrink-0">
                      {item.tag}
                    </span>
                  )}
                </div>
                <p className="text-sm sm:text-base text-text-secondary mt-2 leading-relaxed">{item.description}</p>
              </Link>
        ...
        {/* Optional: Add a subtle fade at the bottom when content is scrollable */}
        {items.length > 4 && (
          <div className="absolute bottom-0 left-0 right-0 h-4 bg-gradient-to-t from-surface to-transparent pointer-events-none rounded-b-md" />
        )}
      </div>
    </div>
  </motion.div>
        ...
        <header className={`fixed top-0 left-0 w-full bg-accent dark:bg-background text-accent-text dark:text-text-primary border-b-4 border-border dark:border-accent z-50 font-body transition-all duration-300 ease-in-out ${isNavVisible ? 'translate-y-0' : '-translate-y-full'}`}>
                                ...
                                {item.dropdown ? (
                                    <>
                                        <button
                                            ...
                                            className="flex items-center gap-1.5 text-base font-bold transition-colors hover:opacity-70"
                                        >
            ...
            {isMenuOpen && (
                <div className="md:hidden bg-accent dark:bg-background border-t border-border dark:border-accent">
                    <nav className="flex flex-col items-center p-6 space-y-4 max-h-[80vh] overflow-y-auto">
                        {navItems.map((item) => (
                            item.dropdown ? (
                                <div key={item.label} className="text-center w-full pb-4 border-b border-border/20 dark:border-accent/20">
                                    ...
                                    <div className="space-y-4 max-h-60 overflow-y-auto">
                                        {item.dropdown.map(subItem => {
                                            const isBlog = item.label === "Blog Posts";
                                            return (
                                                <Link
                                                    key={subItem.title}
                                                    to={subItem.href}
                                                    className={`block text-sm text-accent-text/80 dark:text-text-primary/80 hover:text-accent-text dark:hover:text-text-primary px-2 py-1 ${isBlog ? 'text-left' : ''}`}
                                                    onClick={() => setIsMenuOpen(false)}
                                                >
                                                    <div className={`flex flex-col gap-1 ${isBlog ? 'items-start' : 'items-center'}`}>
                                ...
                                <Link key={item.label} to={item.to} className="text-lg font-bold" onClick={() => setIsMenuOpen(false)}>
                                    {item.label}
                                </Link>
                            )
                        ))}
